---
title: "hilic-negative-sorghum-jan2"
format: html
editor: visual
cache: true
toc: true
...
## hilic negative sorghum everything
...
input_hilic_negative <- "~/Downloads/DEC17/DOE_dec_9_hilic_negative_voi_AND_adjusted_metadata_w_labels.csv"
quantifications_with_metadata = fread(file = input_hilic_negative)
...
hilic.negative.sorgh.raw <- test %>%
  left_join(file1,by="original") 

hilic.negative.sorgh <- hilic.negative.sorgh.raw %>%
  select(identifier,concentration,contains("X"))

paste0("Number of features in hilic+ sorghum:", length(colnames(hilic.negative.sorgh))-2)
...
metabolite_cols <- setdiff(names(hilic.negative.sorgh.raw), metadata_cols)

# Now let's count non-zero values for each feature, grouped by identifier and concentration
feature_nonzero_counts <- hilic.negative.sorgh.raw %>%
...
total_groups <- n_distinct(hilic.negative.sorgh.raw[c("identifier", "concentration")])
...
hilic.negative.sorgh.filtered <- hilic.negative.sorgh.raw %>%
...
# Assuming your dataframe is called 'hilic.negative.set'

# Step 1: Get column names that start with 'X'
x_columns <- grep("^X", names(hilic.negative.sorgh.filtered), value = TRUE)

# Step 2: Among X columns, identify which ones have zero variance
zero_var_x_cols <- sapply(hilic.negative.sorgh.filtered[x_columns], has_zero_variance)
...
if (!is.null(cols_to_remove) && length(cols_to_remove) > 0) {
  ...
  hilic.negative.sorgh.filtered <- hilic.negative.sorgh.filtered[, !names(hilic.negative.set) %in% cols_to_remove]
}


metabolite_cols <- grep("^X\\d+", names(hilic.negative.sorgh.filtered), value = TRUE)

# Calculate PCA on scaled metabolite data
pca_result <- prcomp(hilic.negative.sorgh.filtered[, metabolite_cols], scale. = TRUE)
...
join3 <- hilic.negative.sorgh.filtered %>%
...
pdf("~/Desktop/export_yeast_metabolite/hilic-neg-sorgh-mad-ratios.pdf")
hist(mad_ratios)
abline(v=3)
dev.off()
...
outliers <- detect_pca_outliers(pca_result, hilic.negative.sorgh.filtered)
...
# Step 1: Get column names that start with 'X'
x_columns <- grep("^X", names(clean_data), value = TRUE)

# Step 2: Among X columns, identify which ones have zero variance
zero_var_x_cols <- sapply(clean_data[x_columns], has_zero_variance)
...
ggsave("~/Desktop/export_yeast_metabolite/sorgh_hilic_neg_pc1_vs_pc2.png",dpi=300,
...
ggsave("~/Desktop/export_yeast_metabolite/sorgh_hilic_neg_pc3_vs_pc4.png",dpi=300,
       width = 20,height = 10,units = "in")



saveRDS(out_file,
        "~/Desktop/export_yeast_metabolite/clean_data_no_outliers_sorgh_hilic_neg.rds")
