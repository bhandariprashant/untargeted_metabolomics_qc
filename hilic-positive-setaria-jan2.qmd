---
title: "hilic-positive-setaria-jan2"
format: html
editor: visual
cache: true
toc: true
...
## hilic Positive setaria everything
...
cols_with_pattern <- colnames(quantifications_with_metadata)[str_detect(colnames(quantifications_with_metadata), "^HILIC_Set_|^Set_")]
...
hilic.positive.set.raw <- test %>%
  left_join(file1,by="original") 

hilic.positive.set.raw <- hilic.positive.set.raw %>%
  mutate(identifier=str_replace_all(identifier,"_HILIC","")) %>%
  mutate(identifier=str_replace_all(identifier,"setaria", "Setaria"))

hilic.positive.set <- hilic.positive.set.raw %>%
  select(identifier,concentration,contains("X"))

paste0("Number of features in hilic+ Setaria:", length(colnames(hilic.positive.set))-2)
...
metabolite_cols <- setdiff(names(hilic.positive.set.raw), metadata_cols)

# Now let's count non-zero values for each feature, grouped by identifier and concentration
feature_nonzero_counts <- hilic.positive.set.raw %>%
...
total_groups <- n_distinct(hilic.positive.set.raw[c("identifier", "concentration")])
...
hilic.positive.set.filtered <- hilic.positive.set.raw %>%
...
x_columns <- grep("^X", names(hilic.positive.set.filtered), value = TRUE)

# Step 2: Among X columns, identify which ones have zero variance
zero_var_x_cols <- sapply(hilic.positive.set.filtered[x_columns], has_zero_variance)
...
if (!is.null(cols_to_remove) && length(cols_to_remove) > 0) {
  ...
  hilic.positive.set.filtered <- hilic.positive.set.filtered[, !names(hilic.positive.set) %in% cols_to_remove]
}


metabolite_cols <- grep("^X\\d+", names(hilic.positive.set.filtered), value = TRUE)

# Calculate PCA on scaled metabolite data
pca_result <- prcomp(hilic.positive.set.filtered[, metabolite_cols], scale. = TRUE)
...
join3 <- hilic.positive.set.filtered %>%
...
pdf("~/Desktop/export_yeast_metabolite/hilic-pos-set-mad-ratios.pdf")
hist(mad_ratios)
abline(v=3)
dev.off()
...
outliers <- detect_pca_outliers(pca_result, hilic.positive.set.filtered)
...
ggsave("~/Desktop/export_yeast_metabolite/set_hilic_pos_pc1_vs_pc2.png",dpi=300,
...
ggsave("~/Desktop/export_yeast_metabolite/set_hilic_pos_pc3_vs_pc4.png",dpi=300,
       width = 20,height = 10,units = "in")



saveRDS(out_file,
        "~/Desktop/export_yeast_metabolite/clean_data_no_outliers_set_hilic_pos.rds")
