---
title: "rplc-negative-sorghum-jan2"
format: html
editor: visual
cache: true
toc: true
...
## RPLC negative sorghum everything
...
cols_with_pattern <- colnames(quantifications_with_metadata)[str_detect(colnames(quantifications_with_metadata), "^RPLC_Sorg_|^Sorg_")]
...
rplc.negative.sorgh.raw <- test %>%
  left_join(file1,by="original") 

rplc.negative.sorgh <- rplc.negative.sorgh.raw %>%
  select(identifier,concentration,contains("X"))

paste0("Number of features in RPLC+ sorghum:", length(colnames(rplc.negative.sorgh))-2)
...
metabolite_cols <- setdiff(names(rplc.negative.sorgh.raw), metadata_cols)

# Now let's count non-zero values for each feature, grouped by identifier and concentration
feature_nonzero_counts <- rplc.negative.sorgh.raw %>%
...
total_groups <- n_distinct(rplc.negative.sorgh.raw[c("identifier", "concentration")])
...
rplc.negative.sorgh.filtered <- rplc.negative.sorgh.raw %>%
...
x_columns <- grep("^X", names(rplc.negative.sorgh.filtered), value = TRUE)

# Step 2: Among X columns, identify which ones have zero variance
zero_var_x_cols <- sapply(rplc.negative.sorgh.filtered[x_columns], has_zero_variance)
...
if (!is.null(cols_to_remove) && length(cols_to_remove) > 0) {
  ...
  rplc.negative.sorgh.filtered <- rplc.negative.sorgh.filtered[, !names(rplc.negative.sorgh) %in% cols_to_remove]
}


metabolite_cols <- grep("^X\\d+", names(rplc.negative.sorgh.filtered), value = TRUE)

# Calculate PCA on scaled metabolite data
pca_result <- prcomp(rplc.negative.sorgh.filtered[, metabolite_cols], scale. = TRUE)
...
join3 <- rplc.negative.sorgh.filtered %>%
...
pdf("~/Desktop/export_yeast_metabolite/rplc-neg-sorgh-mad-ratios.pdf")
hist(mad_ratios)
abline(v=3)
dev.off()
...
outliers <- detect_pca_outliers(pca_result, rplc.negative.sorgh.filtered)
...
out_file <- join5[, c("identifier", "plate", "concentration", metabolite_cols)][-outlier_indices, ] %>% distinct()
...
ggsave("~/Desktop/export_yeast_metabolite/sorgh_rplc_neg_pc1_vs_pc2.png",dpi=300,
...
ggsave("~/Desktop/export_yeast_metabolite/sorgh_rplc_neg_pc3_vs_pc4.png",dpi=300,
       width = 20,height = 10,units = "in")



saveRDS(out_file,
        "~/Desktop/export_yeast_metabolite/clean_data_no_outliers_sorgh_rplc_neg.rds")
