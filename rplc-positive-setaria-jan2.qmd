---
title: "rplc-positive-setaria-jan2"
format: html
editor: visual
cache: true
toc: true
...
metabolite_cols <- setdiff(names(rplc.positive.set.raw), metadata_cols)

# Now let's count non-zero values for each feature, grouped by identifier and concentration
feature_nonzero_counts <- rplc.positive.set.raw %>%
...
total_groups <- n_distinct(rplc.positive.set.raw[c("identifier", "concentration")])
...
rplc.positive.set.filtered <- rplc.positive.set.raw %>%
...
x_columns <- grep("^X", names(rplc.positive.set.filtered), value = TRUE)

# Step 2: Among X columns, identify which ones have zero variance
zero_var_x_cols <- sapply(rplc.positive.set.filtered[x_columns], has_zero_variance)
...
if (!is.null(cols_to_remove) && length(cols_to_remove) > 0) {
  ...
  rplc.positive.set.filtered <- rplc.positive.set.filtered[, !names(rplc.positive.set) %in% cols_to_remove]
}


metabolite_cols <- grep("^X\\d+", names(rplc.positive.set.filtered), value = TRUE)

# Calculate PCA on scaled metabolite data
pca_result <- prcomp(rplc.positive.set.filtered[, metabolite_cols], scale. = TRUE)
...
join3 <- rplc.positive.set.filtered %>%
...
pdf("~/Desktop/export_yeast_metabolite/rplc-pos-set-mad-ratios.pdf")
hist(mad_ratios)
abline(v=3)
dev.off()
...
outliers <- detect_pca_outliers(pca_result, rplc.positive.set.filtered)
...
ggsave("~/Desktop/export_yeast_metabolite/set_rplc_pos_pc1_vs_pc2.png",dpi=300,
...
ggsave("~/Desktop/export_yeast_metabolite/set_rplc_pos_pc3_vs_pc4.png",dpi=300,
       width = 20,height = 10,units = "in")



saveRDS(out_file,
        "~/Desktop/export_yeast_metabolite/clean_data_no_outliers_set_rplc_pos.rds")
