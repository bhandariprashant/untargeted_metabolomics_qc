---
title: "hilic-positive-sorghum-jan2"
format: html
editor: visual
cache: true
toc: true
...
## hilic Positive sorghum everything

```{r,warning=FALSE,echo=FALSE,message=FALSE}
#lets look at hilic everything 
library(tidyverse)
library(collapse)
library(arrow)
library(data.table)
...
input_hilic_positive <- "~/Downloads/DEC17/DOE_dec_9_hilic_positive_voi_AND_adjusted_metadata_w_labels.csv"
quantifications_with_metadata = fread(file = input_hilic_positive)


doubleGold <- quantifications_with_metadata %>%
  select(feature_mass, rt,contains("Sorg"))
...
cols_with_pattern <- colnames(quantifications_with_metadata)[str_detect(colnames(quantifications_with_metadata), "^HILIC_Sorg_|^Sorg_")]

extracted_info <- data.frame(
  original = cols_with_pattern,
  sorg_number = str_extract(cols_with_pattern, "(?<=Sorg_)\\d+"),
  ...
  concentration = str_extract(cols_with_pattern, "\\d+(?=_TCM)"),
  identifier = str_extract(cols_with_pattern, "TCM_\\d+")
)
...
extracted_info.na <- extracted_info.na %>%
  mutate(
    ...
    concentration = case_when(
      is.na(concentration) & str_detect(original, "(40|100)_") ~ 
        str_extract(original, "(40|100)(?=_)"),
      TRUE ~ concentration
    ),
...
file1 <- file1 %>%
  mutate(identifier = str_replace_all(identifier, "_presshi", ""))

test$original <- rownames(test)

hilic.positive.sorgh.raw <- test %>%
  left_join(file1,by="original") 


hilic.positive.set <- hilic.positive.sorgh.raw %>%
  select(identifier,concentration,contains("X"))

paste0("Number of features in hilic+ sorghum:", length(colnames(hilic.positive.set))-2)
...
metadata_cols <- colnames(extracted_info)

metabolite_cols <- setdiff(names(hilic.positive.sorgh.raw), metadata_cols)

# Now let's count non-zero values for each feature, grouped by identifier and concentration
feature_nonzero_counts <- hilic.positive.sorgh.raw %>%
...
total_groups <- n_distinct(hilic.positive.sorgh.raw[c("identifier", "concentration")])
...
hilic.positive.sorgh.filtered <- hilic.positive.sorgh.raw %>%
  select(all_of(c(metadata_cols, features_to_keep)))

saveRDS(hilic.positive.sorgh.filtered, "~/Desktop/export_yeast_metabolite/hilic.positive.sorgh.filtered.rds")
...
# Assuming your dataframe is called 'hilic.positive.set'

# Step 1: Get column names that start with 'X'
x_columns <- grep("^X", names(hilic.positive.sorgh.filtered), value = TRUE)

# Step 2: Among X columns, identify which ones have zero variance
zero_var_x_cols <- sapply(hilic.positive.sorgh.filtered[x_columns], has_zero_variance)
...
if (!is.null(cols_to_remove) && length(cols_to_remove) > 0) {
  ...
  hilic.positive.sorgh.filtered <- hilic.positive.sorgh.filtered[, !names(hilic.positive.set) %in% cols_to_remove]
}


metabolite_cols <- grep("^X\\d+", names(hilic.positive.sorgh.filtered), value = TRUE)

# Calculate PCA on scaled metabolite data
pca_result <- prcomp(hilic.positive.sorgh.filtered[, metabolite_cols], scale. = TRUE)
...
library(stringdist)
files <- list.files(
  path = "~/Downloads/INJECTIONS/InjectionListsHILIC",
  pattern = ".csv",
  full.names = TRUE
)
...
processed_data2$File_Name <- ifelse(
  !startsWith(processed_data2$File_Name, "hilic_"),
  paste0("hilic_", processed_data2$File_Name),
  processed_data2$File_Name
)


join3 <- hilic.positive.sorgh.filtered %>%
...
high_variation <- technical_variation %>%
  filter(mad_ratio >= 3|  # Top 25% of MAD ratios
           mean_injection_correlation > 0.99)         # Notable injection correlation

high_mad <- technical_variation %>%
  filter(mad_ratio >= 3)

high_cor <- technical_variation %>%
 filter(mean_injection_correlation > 0.99)

join4 <- join3 %>%
  select(-any_of(high_mad$metabolite)) 

pdf("~/Desktop/export_yeast_metabolite/hilic-pos-sorgh-mad-ratios.pdf")
hist(mad_ratios)
abline(v=3)
dev.off()
...
outliers <- detect_pca_outliers(pca_result, hilic.positive.sorgh.filtered)
...
ggsave("~/Desktop/export_yeast_metabolite/sorgh_hilic_pos_pc1_vs_pc2.png",dpi=300,
...
ggsave("~/Desktop/export_yeast_metabolite/sorgh_hilic_pos_pc3_vs_pc4.png",dpi=300,
       width = 20,height = 10,units = "in")



saveRDS(out_file,
        "~/Desktop/export_yeast_metabolite/clean_data_no_outliers_sorgh_hilic_pos.rds")
